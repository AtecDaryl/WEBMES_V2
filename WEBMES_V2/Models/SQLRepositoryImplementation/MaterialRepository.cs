...
namespace WEBMES_V2.Models.SQLRepositoryImplementation
{
    public class MaterialRepository : IMaterialRepository
    {
        ...
        public async Task<IEnumerable<MaterialStagingHistoryDTO>>? GetMaterialStagingHistoryDTOs(StageLot stageLot)
        {
            await using SqlConnection sqlConnection = _dapperConnection.CreateConnection();

            var materialHistoryDetails = await sqlConnection.QueryAsync<MaterialStagingHistoryDTO>(
                                                                            MaterialStaging.usp_MTL_Material_History,
                                                                            new
                                                                            {
                                                                                MaterialType = stageLot.MaterialType,
                                                                                CustomerCode = stageLot.CustomerCode,
                                                                                DateFrom = stageLot.DateFrom,
                                                                                DateTo = stageLot.DateTo,
                                                                                Mode = stageLot.Mode
                                                                            },
        ...
        public async Task<IEnumerable<MaterialHistoryDTO>> Get_Material_Dashboard(StageLot stageLot)
        {
            ...
            var materialHistoryDetails = await sqlConnection.QueryAsync<MaterialHistoryDTO>(
                                                                            MaterialStaging.usp_MTL_Material_Dashboard,
                                                                            new
                                                                            {
                                                                                MaterialType = stageLot.MaterialType
                                                                            },
