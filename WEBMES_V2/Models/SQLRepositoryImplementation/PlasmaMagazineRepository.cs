...
namespace WEBMES_V2.Models.SQLRepositoryImplementation
{
    public class PlasmaMagazineRepository : IPlasmaMagazineRepository
    {
        ...
        public async Task<int> InsertGetMachine(Trn_InsertMachineDTO Trn_InsertMachineDTO)
        {
             var mapMachineDTO = _mapper.Map<TrnMachinePerStage>(Trn_InsertMachineDTO);
             var insertedDetail = await _mesAtecContext
                                                .TrnMachinePerStages
                                                .AddAsync(mapMachineDTO);

            await _mesAtecContext.SaveChangesAsync();                         
            
            return mapMachineDTO.Id;
        }
        public async Task<bool> CheckMachineIfExist(Trn_InsertMachineDTO machineDetail)
        {
            var isExist = await _mesAtecContext
                                  .TrnMachinePerStages
                                  .AnyAsync(machine => machine.TrnTransactionId == machineDetail.TrnTransactionId &&
                                                       machine.StageId == machineDetail.StageId);

            return isExist;
        }

        public async Task<bool?> CheckMachineExceed15Mins(StageLot stageLot)
        {
            var fifteenMinutes = TimeSpan.FromMinutes(15);

             var isExist =  _mesAtecContext
                                            .TrnMachinePerStages
                                            .Where(x => x.MachineId == stageLot.MachineCode);

            var maxDate = isExist.Max(d => d.TrackOutDateTime);

            if (maxDate == null)
                return null;


            var difference = DateTime.Now.Subtract(maxDate.Value);
